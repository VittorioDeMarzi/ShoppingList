...
submitButton.addEventListener("click", (ev) => {
  ...
  //fetch('localhost:8080/api/v1/shoppinglist')
  fetch("http://localhost:8080/api/v1/shoppinglist", {
    method: "POST",
    headers: {
...
function getAllItemsToBuy() {
  ...
  fetch("http://localhost:8080/api/v1/shoppinglist/status/bought=false")
    .then((res) => res.json())
    .then((data) => {
...
function getAllBoughtsItems() {
  ...
  fetch("http://localhost:8080/api/v1/shoppinglist/status/bought=true")
    .then((res) => res.json())
    .then((data) => {
      data.forEach((element) => {
        const container = document.createElement("article");
...
function deleteProduct(id) {
  ...
  fetch("http://localhost:8080/api/v1/shoppinglist/" + id, {
    method: "DELETE",
  }).then((res) => {
    if (res.ok) {
      getAllItemsToBuy();
